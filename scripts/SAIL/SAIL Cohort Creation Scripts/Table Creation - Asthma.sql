------------------------ TABLE CREATION FOR PROJECT 1387 -- Asthma -------------- 07/03/2022
...
-------- select first incident code 
SELECT COUNT(DISTINCT wgec.ALF_PE) FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec 
INNER JOIN sailw1387v.SS_ASTHMA_CODES ASTHMA ON ASTHMA.EVENT_CD = wgec.EVENT_CD 
...
AND (YEAR(wgec.EVENT_DT) > YEAR(wgec.WOB)) 
AND ASTHMA.EVENT_CD = wgec.EVENT_CD 

-- Select the first diagnisis for each ALF
DECLARE GLOBAL TEMPORARY TABLE MIN_ASTHMA_DIAG_SETUP AS (
...
FROM SESSION.MIN_ASTHMA_DIAG_SETUP
GROUP BY ALF_PE
ORDER BY ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

-- Delete any events before 1930 as they are very likely incorrect entries 
DELETE FROM SESSION.MIN_ASTHMA_DIAG WHERE FIRST_DIAGNOSIS_DT < '01/01/1930'  --// Optional but was done to try and improve data quality

-- prototype cohort for generating WIMD and Ethnicity extraction 
DECLARE GLOBAL TEMPORARY TABLE prototype_cohort_ASTHMA AS (
SELECT DISTINCT wgec.ALF_PE ,per.WOB, per.DOD, per.GNDR_CD, FIRST_DIAGNOSIS_DT
FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec 
LEFT JOIN SAIL1387V.WDSD_AR_PERS_20220307 per  ON per.ALF_PE = wgec.ALF_PE
LEFT JOIN SESSION.MIN_ASTHMA_DIAG mid ON wgec.ALF_PE = mid.ALF_PE
INNER JOIN sailw1387v.SS_ASTHMA_CODES ASTHMA ON ASTHMA.EVENT_CD = wgec.EVENT_CD 
...
FROM SESSION.prototype_cohort_ASTHMA rt
...
DROP TABLE sailw1387V.SS_ASTHMA_LSOA_ALL

-- Create perminent table
CREATE TABLE sailw1387V.SS_ASTHMA_LSOA_ALL(
ALF_PE BIGINT
...
INSERT INTO sailw1387V.SS_ASTHMA_LSOA_ALL
SELECT DISTINCT 
ALF_PE
, WIMD_2011_QUINTILE 
, START_DATE 
...
SELECT * FROM sailw1387V.SS_ASTHMA_LSOA_ALL
...
DECLARE GLOBAL TEMPORARY TABLE LSOA_ASTHMA AS (
...
DROP TABLE sailw1387V.SS_ASTHMA_LSOA

-- Create perminent table 
CREATE TABLE sailw1387V.SS_ASTHMA_LSOA(
ALF_PE BIGINT
, WIMD_2011_QUINTILE int
, START_DATE DATE
, END_DATE DATE
...
INSERT INTO sailw1387V.SS_ASTHMA_LSOA
SELECT DISTINCT 
ALF_PE
, WIMD_2011_QUINTILE 
, START_DATE 
...
INNER JOIN SESSION.prototype_cohort_ASTHMA rt ON rt.ALF_PE = wgec.ALF_PE 
...
-- Combine tables
DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON AS (
...
INNER JOIN SESSION.prototype_cohort_ASTHMA rt ON rt.ALF_PE = PS.ALF_PE 
...
-- Combine tables 
DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON_PEDW AS (
SELECT DISTINCT ALF_PE,  Ethnicity_Code
FROM 
...
SELECT ALF_PE, COUNT(ALF_PE) AS COUNTS FROM SESSION.ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON_PEDW
GROUP BY ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;
...
DELETE FROM SESSION.ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON_PEDW WHERE ALF_PE IN(SELECT ALF_PE FROM SESSION.ETHNICITY_DUPLICATES_PEDW WHERE COUNTS > '1')

--- Combine Hospital and GP tables 
DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_DUPLICATES AS (
SELECT ALF_PE, COUNT(ALF_PE) AS COUNTS FROM SESSION.ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON
GROUP BY ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA_BASIC AS (
...
FROM SESSION.prototype_cohort_ASTHMA pci
LEFT JOIN SESSION.ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON ECM ON ECM.ALF_PE = pci.ALF_PE
LEFT JOIN SESSION.ETHNICITY_ASTHMA_BY_DATE_AND_MOST_COMMON_PEDW ECMP ON ECMP.ALF_PE = pci.ALF_PE
ORDER BY ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA AS (
SELECT DISTINCT ALF_PE, Ethnicity_Code
...
FROM SESSION.ETHNICITY_ASTHMA_BASIC EIB
...
--- find not stated 
DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA_NULLS AS (
SELECT * FROM SESSION.ETHNICITY_ASTHMA WHERE Ethnicity_Code IS NULL
) WITH DATA WITH replace ON commit PRESERVE ROWS ;
...
DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA_NOTSTATED AS (
SELECT ecn.ALF_PE, ecm.ETHNICITY_CODE FROM SESSION.ETHNICITY_ASTHMA_NULLS ecn 
...
SELECT * FROM SESSION.ETHNICITY_ASTHMA_NOTSTATED

---- join into main table 
DECLARE GLOBAL TEMPORARY TABLE ETHNICITY_ASTHMA_V2 AS (
...
FROM SESSION.ETHNICITY_ASTHMA ec
LEFT JOIN SESSION.ETHNICITY_ASTHMA_NOTSTATED ecn ON ecn.ALF_PE = ec.ALF_PE AND ecn.ETHNICITY_CODE = ec.ETHNICITY_CODE 
ORDER BY ec.ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

SELECT * FROM SESSION.ETHNICITY_ASTHMA_V2
------------------------------------------------------------------------------------DEMOGRAPHIC TABLE--------------------------------------------------------------------------------------------------

-- Select latest registration date per ALF
DECLARE GLOBAL TEMPORARY TABLE Registration_ASTHMA AS (
SELECT DISTINCT pci.ALF_PE, 
...
INNER JOIN SESSION.prototype_cohort_ASTHMA pci ON wap.ALF_PE = pci.alf_PE
...
DECLARE GLOBAL TEMPORARY TABLE MIN_ASTHMA_PEDW AS (
...
INNER JOIN SESSION.prototype_cohort_ASTHMA gp ON gp.ALF_PE = ps.ALF_PE
INNER JOIN SAILW1387V.SS_ASTHMA_ICDCODES CP ON CP.CODE = ep.DIAG_CD_123
WHERE YEAR(ep.EPI_STR_DT) > (YEAR(WOB) + 1)
GROUP BY ps.ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

SELECT * FROM SESSION.MIN_ASTHMA_PEDW
ORDER BY ALF_PE

SELECT COUNT(ALF_PE), COUNT(DISTINCT ALF_PE) FROM SESSION.MIN_ASTHMA_PEDW

-----  Consrtuct Final demographic table 
DECLARE GLOBAL TEMPORARY TABLE DEMOGRAPHIC_TABLE_ASTHMA AS (SELECT DISTINCT wgec.ALF_PE, per.WOB, Ethnicity_Code, ETHNICTITY_DESCRIPTION_5, ETHNICTITY_DESCRIPTION_11, ETHNICTITY_DESCRIPTION_21, YEAR(per.WOB) AS YEAR_OF_BIRTH, lsoa.WIMD_2011_QUINTILE, wgec.GNDR_CD, per.DOD, FIRST_DIAGNOSIS_DT, FOLLOW_UP_START , FOLLOW_UP_END , INCIDENT, PREVALENT,
...
FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec 
LEFT JOIN SAIL1387V.WDSD_AR_PERS_20220307 per  ON per.ALF_PE = wgec.ALF_PE
LEFT JOIN SESSION.MIN_ASTHMA_DIAG ASTHMAd ON wgec.ALF_PE = ASTHMAd.ALF_PE
LEFT JOIN sailw1387V.SS_ASTHMA_LSOA lsoa ON wgec.ALF_PE = lsoa.ALF_PE
INNER JOIN sailw1387v.SS_ASTHMA_CODES ASTHMA ON wgec.EVENT_CD = ASTHMA.EVENT_CD
LEFT JOIN SESSION.ETHNICITY_ASTHMA et ON et.alf_pe = wgec.alf_pe
LEFT JOIN SESSION.Registration_ASTHMA ri ON ri.ALF_PE = wgec.ALF_PE 
LEFT JOIN SESSION.MIN_ASTHMA_PEDW mp ON mp.ALF_PE = wgec.ALF_PE
WHERE (wgec.GNDR_CD = '1' OR wgec.GNDR_CD = '2')  --// only alows for a clearly defined gender code - delete if not applicable 
AND (DOD > '01/01/2004' OR DOD IS NULL) --// change to project start date
AND FIRST_DIAGNOSIS_DT > per.WOB
AND ASTHMA.EVENT_CD = wgec.EVENT_CD
AND (FOLLOW_UP_END > '31/12/2003') --// change to day before project start date
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

SELECT Count(DISTINCT ALF_PE) FROM sailw1387v.SS_DEMOGRAPHIC_TABLE_COPD WHERE 

-- Code to drop table before adding alterations 
DROP TABLE sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA

-- Create perminent table 
CREATE TABLE sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA(
 ALF_PE BIGINT, 
 WOB DATE, 
...
INSERT INTO sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA
SELECT DISTINCT ALF_PE , 
 ...
 FOLLOW_UP_END,
 EARLIEST_MENTION
FROM SESSION.DEMOGRAPHIC_TABLE_ASTHMA

SELECT * FROM sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA
ORDER BY ALF_PE

SELECT COUNT(DISTINCT ALF_PE) FROM sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA



------------------------------------------------------------------------------------CONDITIONS TABLE--------------------------------------------------------------------------------------------------
--Create Start and End date fields 
DECLARE GLOBAL TEMPORARY TABLE FIRST_EVENTS_ASTHMA AS (
SELECT DISTINCT wgec.ALF_PE, wgec.EVENT_CD, MIN(EVENT_DT) AS START_DATE, MAX(EVENT_DT) AS LATEST_EVENT
FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec
INNER JOIN sailw1387v.SS_ASTHMA_CODES ASTHMA ON ASTHMA.EVENT_CD = wgec.EVENT_CD 
INNER JOIN sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA DTI ON DTI.ALF_PE = wgec.ALF_PE
GROUP BY wgec.ALF_PE, wgec.EVENT_CD
ORDER BY wgec.ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;

SELECT * FROM SESSION.FIRST_EVENTS_ASTHMA

-- Create table of all events that have occured per individual
DECLARE GLOBAL TEMPORARY TABLE CONDITIONS_TABLE_ASTHMA AS (SELECT wgec.ALF_PE, wgec.EVENT_CD, wgec.EVENT_DT, START_DATE, LATEST_EVENT, INCIDENT, PREVALENT, 'ASTHMA' AS CONDITION_TYPE, 'GP' AS DATASET_SOURCE 
FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec 
LEFT JOIN SESSION.FIRST_EVENTS_ASTHMA sdi ON sdi.ALF_PE = wgec.ALF_PE AND sdi.EVENT_CD = wgec.EVENT_CD 
INNER JOIN sailw1387v.SS_ASTHMA_CODES ASTHMA ON ASTHMA.EVENT_CD = wgec.EVENT_CD
INNER JOIN sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA DTI ON DTI.ALF_PE = wgec.ALF_PE
WHERE (wgec.GNDR_CD = '1' OR wgec.GNDR_CD = '2') --// only alows for a clearly defined gender code - delete if not applicable 
) WITH DATA WITH replace ON commit PRESERVE ROWS ;
...
-- Code to drop table before adding alterations 
DROP TABLE sailw1387v.SS_CONDITIONS_TABLE_ASTHMA

-- Create perminent table 
CREATE TABLE sailw1387v.SS_CONDITIONS_TABLE_ASTHMA(
ALF_PE BIGINT
, EVENT_CD VARCHAR(5)
, EVENT_DT date 
, START_DATE DATE
...
INSERT INTO sailw1387v.SS_CONDITIONS_TABLE_ASTHMA
SELECT ALF_PE
, EVENT_CD
, EVENT_DT
, START_DATE
...
, CONDITION_TYPE
, DATASET_SOURCE
FROM SESSION.CONDITIONS_TABLE_ASTHMA
...
DROP TABLE sailw1387v.ASTHMA_MEDICATION_CODES

-- Create table of medication codes 
CREATE TABLE sailw1387v.ASTHMA_MEDICATION_CODES(
...
SELECT * FROM sailw1387v.ASTHMA_MEDICATION_CODES
...
INNER JOIN sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA ssd ON ssd.ALF_PE = wlgp.ALF_PE 
INNER JOIN sailw1387v.ASTHMA_MEDICATION_CODES cmc ON cmc.READCODE = wlgp.EVENT_CD 
...
DECLARE GLOBAL TEMPORARY TABLE Medication_Catagory_Asthma AS(
...
SELECT COUNT(DISTINCT ALF_PE) FROM SESSION.Medication_Catagory_Asthma WHERE alf_pe NOT IN(SELECT ALF_PE FROM SESSION.Medication_Catagory_Per_ALF)

SELECT COUNT(DISTINCT ALF_PE) FROM SESSION.Medication_Catagory_Asthma 
...
FROM SESSION.Medication_Catagory_Asthma
...
SELECT 
(SUM(LAMA_FLAG)/'572271')*100 AS LAMA
,(SUM(LABA_FLAG)/'572271')*100 AS LABA
,(SUM(SABA_FLAG)/'572271')*100 AS SABA
,(SUM(LABAICS_FLAG)/'572271')*100 AS LABAICS
,(SUM(SABASAMA_FLAG)/'572271')*100 AS SABASAMA
,(SUM(ICS_FLAG)/'572271')*100 AS ICS
,(SUM(ABX_FLAG)/'572271')*100 AS ABX
,(SUM(pde4i_FLAG)/'572271')*100 AS pde4i
,(SUM(OCS_FLAG)/'572271')*100 AS OCS
,(SUM(THEOPHYLLINE_FLAG)/'572271')*100 AS THEOPHYLLINE
,(SUM(LABALAMA_FLAG)/'572271')*100 AS LABALAMA
,(SUM(TRIPLE_FLAG)/'572271')*100 AS TRIPLE
,(SUM(SAMA_FLAG)/'572271')*100 AS SAMA
FROM SESSION.Medication_Catagory_Asthma

------------------------------------------------------------------------------------ GINA STEPS  --------------------------------------------------------------------------------------------------
-- Create table with flags of each medication per ALF - EXPERIMENTAL
/* DECLARE GLOBAL TEMPORARY TABLE STEP_1_AND_2 AS(
SELECT ALF_PE FROM SESSION.Medication_Catagory_ASTHMA
WHERE SABA_FLAG > 0 OR ICS_FLAG > 0 
) WITH DATA WITH replace ON commit PRESERVE ROWS;

DECLARE GLOBAL TEMPORARY TABLE STEP_3 AS(
SELECT ALF_PE  FROM SESSION.Medication_Catagory_ASTHMA
WHERE COMBO_FLAG > 0 OR ICS_FLAG > 0 OR LABA_FLAG > 0 OR (LTRA_SCR_FLAG > 0 AND ICS_FLAG > 0)
) WITH DATA WITH replace ON commit PRESERVE ROWS;

DECLARE GLOBAL TEMPORARY TABLE STEP_4_AND_5 AS(
SELECT ALF_PE  FROM SESSION.Medication_Catagory_ASTHMA
WHERE SABA_FLAG > 0 OR ICS_FLAG > 0 
) WITH DATA WITH replace ON commit PRESERVE ROWS;

ALTER TABLE sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA sdta
ADD 
CASE WHEN sdta.ALF_PE IN(SELECT ALF_PE FROM SESSION.STEP_1_AND_2) THEN 1 ELSE 0 END AS GINA_STEP_1_AND_2_FLAG INT,
CASE WHEN sdta.ALF_PE IN(SELECT ALF_PE FROM SESSION.STEP_3) THEN 1 ELSE 0 END AS GINA_STEP_3_FLAG INT,
CASE WHEN sdta.ALF_PE IN(SELECT ALF_PE FROM SESSION.STEP_4_AND_5) THEN 1 ELSE 0 END AS GINA_STEP_4_AND_5_FLAG INT */

------------------------------------------------------------------------------------CONDITIONS TABLE--------------------------------------------------------------------------------------------------
--Create Start and End date fields 
DECLARE GLOBAL TEMPORARY TABLE FIRST_EVENTS_ASTHMA AS (
SELECT DISTINCT wgec.ALF_PE, wgec.EVENT_CD, MIN(EVENT_DT) AS FIRST_EVENT, MAX(EVENT_DT) AS LATEST_EVENT
FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec
INNER JOIN sailw1387v.SS_ASTHMA_CODES ild ON ild.EVENT_CD = wgec.EVENT_CD 
INNER JOIN sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA DTI ON DTI.ALF_PE = wgec.ALF_PE
GROUP BY wgec.ALF_PE, wgec.EVENT_CD
ORDER BY wgec.ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS ;
...
-- Create table of all events that have occured per individual
DECLARE GLOBAL TEMPORARY TABLE CONDITIONS_TABLE_ASTHMA AS (SELECT DISTINCT wgec.ALF_PE, wgec.EVENT_CD, wgec.EVENT_DT, FIRST_EVENT, LATEST_EVENT, INCIDENT, PREVALENT, 'ASTHMA' AS CONDITION_TYPE, 'GP' AS DATASET_SOURCE 
FROM SAIL1387V.WLGP_GP_EVENT_CLEANSED_20220201 wgec
LEFT JOIN SESSION.FIRST_EVENTS_ASTHMA sdi ON sdi.ALF_PE = wgec.ALF_PE AND sdi.EVENT_CD = wgec.EVENT_CD 
INNER JOIN sailw1387v.SS_DEMOGRAPHIC_TABLE_ASTHMA DTI ON DTI.ALF_PE = wgec.ALF_PE
INNER JOIN sailw1387v.SS_ASTHMA_CODES ild ON ild.EVENT_CD = wgec.EVENT_CD
...
SELECT * FROM SESSION.CONDITIONS_TABLE_ASTHMA
...
DECLARE GLOBAL TEMPORARY TABLE ASTHMA_VACCINATION_DATA AS (
...
INNER JOIN sailw1387V.SS_DEMOGRAPHIC_TABLE_ASTHMA scc ON scc.ALF_PE = CWO.ALF_PE 
ORDER BY ALF_PE
) WITH DATA WITH replace ON commit PRESERVE ROWS

DELETE FROM SESSION.ASTHMA_VACCINATION_DATA WHERE AGE_AT_VACCINATION < 1

-- Drop table to add changes if nessisary 
DROP TABLE sailw1387V.SS_ASTHMA_VACCINATION_DATA

-- Create perminent table 
CREATE TABLE sailw1387V.SS_ASTHMA_VACCINATION_DATA(
ALF_PE BIGINT
...
INSERT INTO sailw1387V.SS_ASTHMA_VACCINATION_DATA
SELECT DISTINCT 
ALF_PE 
...
FROM SESSION.ASTHMA_VACCINATION_DATA
ORDER BY ALF_PE
